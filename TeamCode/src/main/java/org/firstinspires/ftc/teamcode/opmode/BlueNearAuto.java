...
public class BlueNearAuto extends StealthOpMode {
    ...
    public void initialize() {
        ...
        toPropRight = drive.trajectoryBuilder(startPose)
                .splineToSplineHeading(new Pose2d(17,47,Math.toRadians(220)),Math.toRadians(0))
                .build();
        toPropLeft = drive.trajectoryBuilder(startPose)
                .splineToSplineHeading(new Pose2d(19,56,Math.toRadians(285)),Math.toRadians(0))
                .build();
        toBoardCenter = drive.trajectoryBuilder(toPropCenter.end())
    ...
    public Command getAutoCommand() {
        if(propPosition == PropProcessor.PropPosition.CENTER)
        {
            return new SequentialCommandGroup(
                ...
                new ParallelCommandGroup(
                    new ArmToSetpoint(arm, 4500),
                    new FollowTrajectoryCommand(drive, toBoardCenter),
                    new InstantCommand(() -> wrist.wristRotate(XeroConstants.WristScorePosition))),
                new InstantCommand(()->finger.moveFinger(XeroConstants.FingerOpenPosition)),
